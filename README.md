...
| 항목 | Before | After | 개선율 |
|---|---|---|---|
| Playbook 수행 시간 | 기준 대비 | **약 50% 단축** | Fact 선택 수집 + 병렬 처리 최적화 |
| 대상 서버 CPU 부하 (Fact 수집 시) | 약 70% | 약 40% | **~43% 감소** |
| 운영 Playbook 수 | — | **50개 이상** | 2024년부터 지속 고도화 |
| VM 프로비저닝 | 수동 SSH 개별 설정 | Ansible 원클릭 자동화 | **인적 오류 제거** |

- 필요한 Fact만 선택적으로 수집하도록 개선 → 대상 서버 CPU 부하 70% → 40%로 감소, Playbook 수행 시간 약 50% 단축
- `serial`, `forks`, `pipelining` 최적화로 대규모 서버 대상 병렬 작업 수행 성능 개선
- VM 프로비저닝·OS 설정·보안 취약점 조치·DNS/NTP·모니터링 에이전트 설치 등 인프라 운영 자동화 체계 구축
