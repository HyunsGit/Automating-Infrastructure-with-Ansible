...
> KakaoCloud 환경에서 Ansible을 사용해 **VM 초기 구성, 보안 강화, 모니터링 에이전트 배포, 인프라 상태 검증**을 자동화한 운영 플레이북 모음입니다.  
> Puppet 기반 구성 관리와 병행하여, Puppet 전환 이전의 초기 프로비저닝과 원타임 작업에 Ansible을 활용합니다.
...
| 항목 | 내용 |
|---|---|
| **대상 환경** | KakaoCloud (Ubuntu 22.04 / Rocky Linux 9) |
...
│   ├── verify/                               # 상태 검증 플레이북
│   ├── dominic/                              # 모니터링·DNS 플레이북
│   ├── hans/                                 # Puppet 전환·인프라 유틸
...
### 1. 표준 VM 초기 투입 (`Default-Playbook-V3.yaml`)

신규 VM이 투입될 때 실행하는 **원스톱 초기화 플레이북**입니다. 아래 작업을 순서대로 수행합니다:
...
> **`TOBE-Default-Playbook-V3.yaml`**: Puppet 전환 완료 후 사용. KC 모니터링 에이전트, 호스트명 변경, Puppet 에이전트 설치, Timezone 동기화만 포함 (나머지는 Puppet이 관리).
...
### 3. 모니터링 스택 배포 (`playbook/middleware/`, `playbook/dominic/`)
...
| `dominic/filebeats-vm-8-install.yaml` | Filebeat 8.17.10 설치 (Object Storage에서 배포) |
...
### 4. 네트워크 / DNS / NTP (`playbook/modify/`, `playbook/dominic/`)
...
| `dominic/update-systemd-resolved-ubuntu.yaml` | systemd-resolved 설정 갱신 (Ubuntu) |
| `dominic/update-systemd-resolved-rocky.yaml` | systemd-resolved 설정 갱신 (Rocky) |
| `dominic/vm/modify-ntp-chronyd.yaml` | chrony NTP 서버 변경 |
...
### 6. Puppet 전환 지원 (`playbook/hans/`)

Ansible로 운영하던 인프라를 Puppet으로 이전하는 작업을 지원하는 플레이북 모음입니다.
...
AWS 자격증명 및 OS 계정 패스워드는 **Ansible Vault** 또는 **HashiCorp Vault**를 통해 관리합니다. 플레이북에서 직접 하드코딩하지 않습니다.
